...
function validarImagen(req, res, next) {
  ...
  if (!/\.(png|jpg)$/i.test(profileImage)) {
...
function checkBoxes(req, res, next) {
  ...
  } else if (!/\.(png|jpg)$/.test(profileImage)) {
    req.success = 'La imagen debe ser en formato PNG o JPG';
    req.type = types.warning;
    next();
...
async function queryUser(req, res, next) {
  ...
  const queryInsert = `INSERT INTO usuarios(nombre, contra, imagen) VALUES('${nencriptado}', '${cencriptada}','${profileImage}')`
...
async function iniciarSesion(req, res, next) {
  ...
  cn.query(inicioSesion, function (err, result, fields) {
    ...
    if (result.length > 0) {
      ...
      req.profileImage = result[0].profileImage
      return next()
    } else {
      req.type = types.warning
      return next()
...
app.post('/registrar', checkBoxes, validarImagen, queryUser, (req, res) => {
...
app.post("/login", validacion, iniciarSesion, (req, res) => {
  ...
  if (req.type === types.warning) {
    ...
    res.render("sesion", { success: req.success, type: req.type, nombre: req.nombre, profileImage: req.profileImage });
  }
});
